...
export const MinesGameComponent: React.FC<Props> = ({ token, refreshWallet }) => {
  ...
  const reveal = async (position: number) => {
    ...
    try {
      ...
      else { setMultiplier(data.currentMultiplier || 1); tone(520 + (data.currentMultiplier || 1) * 60); if (data.isCleared) { setMinePositions(data.minePositions || []); setGameActive(false); setFeedback({ type: 'success', msg: data.message || 'Automatic maximum payout reached.' }); refreshWallet(); } }
    } catch (error: any) { setFeedback({ type: 'error', msg: error.message }); }
    finally { setLoading(false); }
  };
